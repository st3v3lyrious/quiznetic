...
- [ ] M20: Add leaderboard integrity protections (anti-cheat scoring checks, abuse controls, rate limits).
  ...
  - `cleanupOnUserDeleted` Cloud Function written and ready — deletes `users/{uid}` Firestore data when an anonymous user's account is deleted on sign-out.
  - **TODO before/after launch:** Upgrade project `quiznetic-30734` to Blaze plan, then run `cd functions && firebase deploy --only functions` to activate both functions.
