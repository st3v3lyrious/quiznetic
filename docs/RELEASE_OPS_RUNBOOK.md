...
- `ENABLE_APPLE_SIGN_IN` (default: `false`)
  - Purpose: gate Apple sign-in provider until Apple Developer + Firebase credentials are configured.
  - Source: `lib/config/app_config.dart`

> **POST-MVP:** Ads and IAP kill switches (`ENABLE_ADS`, `ENABLE_IAP`, `ENABLE_RESULT_INTERSTITIAL_ADS`, `ALLOW_LIVE_AD_UNITS_IN_DEBUG`, `ENABLE_REWARDED_HINTS`, `ENABLE_PAID_HINTS`) will be re-added to `AppConfig` when monetization is re-enabled. See `docs/MONETIZATION_SETUP.md`.
